...
NOTE: All code in this vignette is unevaluated for Bioconductor release 2.7, because of
last-minute changes to the snpMatrix package by H-T Leung.  You can build this vignette when
using Bioconductor release 2.8, by changing all eval=FALSE to TRUE.

Genotype calls are held in a list of snp.matrix instances:
<<lkd,eval=FALSE>>=
library(ceu1kg)
...
Metadata about the loci are provided in GRanges instances:
<<lkm,eval=FALSE>>=
if (!exists("ceu1kgMeta_20")) data(ceu1kgMeta_20)
ceu1kgMeta_20[1:3]
@
...
genotype plus expression data for these 43 are stored in an `smlSet' container:
<<lkg,eval=FALSE>>=
