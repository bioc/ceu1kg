...
%\VignetteDepends{GGtools, snpMatrix}
...
The 1KG genotypes are available for 41 of these 90 and the associated
genotype plus expression data for these 41 are stored in an `smlSet' container:
<<lkg>>=
data(ceu1kg)
ceu1kg
@

\section{Imputation of 1KG genotypes to 19 `phase II only' CEU individuals}
