...
<<lkreg>>=
C20IMP_REG_SAVE = C20IMP_REG = t(impute.snps(imphm3_1KG_20_mA2, hm3_20gt))
@
...
In the following we will check a single gene for any eQTL by
...
values for the gene used against the two call sets.  We see that mach calls a single homozygote with
low expression, yielding a sharper slope of mean expression against allele count.
<<doplaa,fig=TRUE>>=
par(mfrow=c(2,2))
...
is the model chosen by snpMatrix:
<<lkrul>>=
...
get(bestg, illuminaHumanv1SYMBOL)
@
...
unix.time(runb1 <- unlist(mclapply(1:nrow(C20MLDOSE), function(x) {
   if (x%%1000 == 0) cat(x);pv(ex3[bestg,]~C20MLDOSE[x,intersect(colnames(C20MLDOSE), colnames(ex3))])})))
