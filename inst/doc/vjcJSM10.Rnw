...
Four million Phase II HapMap genotypes for 30 CEU trios are available 
in the Bioconductor \textit{GGdata} package.  We now describe how
...
or not genotyped in HapMap Phase II.  We will focus on chromosome 20 for
illustration.
...
provided in HapMap Phase II.
<<gethm>>=
library(GGdata)
if (!exists("hmceuB36")) data(hmceuB36)
hm20 = hmceuB36[ chrnum("20"), ]
hm20
hm20gt = smList(hm20)[[1]]
hm20gt
dim(hm20gt)
hm20snp = colnames(hm20gt)
@
...
steps can be used to obtain locations for SNP from HapMap phase II:
<<getdds>>=
...
<<chjk>>=
unloc = setdiff(hm20snp, l20names)
length(unloc)
@
For now, drop these loci
<<dropul>>=
badi = match(unloc, colnames(hm20gt))
hm20gt = hm20gt[, -badi]
@
and subset the full set of addresses accordingly:
<<dom>>=
hm20locs = l20addr[hm20snp]
@
...
<<lkm>>=
inbo = intersect(names(hm20locs), names(clocs))
range(hm20locs[inbo] - clocs[inbo])
@

Now we proceed to imputation.  We first partition the 1KG data into targets and predictors;
predictors are those available in both the HapMap Phase II and 1KG panels; targets are all other 1KG loci.
<<gets>>=
X = c1kg_20[ , intersect(colnames(hm20gt), colnames(c1kg_20))]
Y = c1kg_20[ , setdiff(colnames(c1kg_20), colnames(X))]
@
Now impute with all defaults:
<<lkdi>>=
options(digits=3)
args(snp.imputation)
unix.time(imphm2_1KG_20 <- snp.imputation(X, Y, hm20locs[colnames(X)], clocs[colnames(Y)]))
imphm2_1KG_20[1:5]
length(imphm2_1KG_20)
object.size(imphm2_1KG_20)
summary(imphm2_1KG_20)
save(imphm2_1KG_20, file="imphm2_1KG_20.rda")
@
The summary shows that about 42000 targets could not be imputed
